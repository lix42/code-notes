...
const defaultOptions: ThrottlingOptions = {
  leading: true,
  tailing: true,
  jitter: undefined,
};
...
export const flexibleThrottle = (
  ...
  const isLeading = (timeout: number): boolean =>
    leadingTime === 0 || dateNow() - leadingTime > timeout;
  ...
  const setTimer = (timeout: number) => {
    ...
    const thisTimeout = Math.max(
      remainTime + (typeof jitter === "function" ? jitter() : 0),
      0
    );
